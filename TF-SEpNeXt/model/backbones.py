...
from model.shared import ConvBnRelu, ResNorm, AdaResNorm, LayerNorm, BroadcastBlock,TimeFreqSepConvolutions, TimeFreqSepConvNeXtSE
...
class TFSEpNeXt(_BaseBackbone):
    ...
    def __init__(self, in_channels: int = 1, num_classes: int = 10, base_channels: int = 32, depth: int = 19,
                 dropout: float = 0.1, kernel_size: int = 7, drop_path_concat: bool = False, expansion: int =4):
        super(TFSEpNeXt, self).__init__()
        self.dropout = dropout
        self.kernel_size = kernel_size
        ...
        self.conv_layers = nn.Sequential(nn.Conv2d(in_channels, base_channels // 2,  3, stride=2, padding=1),
    ...
    def _make_layers(self, width: int, layer_config: list):
        ...
        for v in layer_config:
            if v == 'N':
                ...
                layers += [nn.Sequential(nn.Conv2d(in_channels=vt, out_channels=vt, kernel_size=2, stride=1, padding=1),
                                        nn.BatchNorm2d(vt),
                                        nn.GELU() )]
            elif v != vt:
