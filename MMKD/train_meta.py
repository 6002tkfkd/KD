...
from setting import (cifar100_teacher_model_name, dogs_teacher_model_name, tinyimagenet_teacher_model_name, dcase_teacher_model_name, teacher_model_path_dict)
...
def parse_option():
    ...
    parser.add_argument('--model_s', type=str, default='resnet8',
                        choices=['resnet8', 'resnet14', 'resnet20', 'resnet32', 'resnet44', 'resnet56', 'resnet110',
                                 ...
                                 'MobileNetV2', 'ShuffleV1', 'ShuffleV2', 'shufflenet_v2_x0_5', 'BEATs', 'TFSEpNeXt'])
    ...
    opt.model_path = '/home/work/HAN3/save/meta/1113_other/students/models'
    opt.tb_path = '/home/work/HAN3/save/meta/1113_other/students/tensorboard'
    ...
    if opt.dataset == 'cifar100':
        ...
        opt.teacher_model_name = dcase_teacher_model_name
    ...
    opt.run_id = time.strftime('%Y%m%d_%H%M%S')
    opt.model_name = opt.model_name + '_' + opt.run_id
...
def load_teacher(model_path, n_cls, model_t, opt=None):
    ...
    if model_t == 'BEATs':
        model = model_dict[model_t](num_classes=n_cls, pretrained=model_path)
        print('==> done')
        return model
    if model_t == 'PaSST':
        input_tdim = 998
        model = model_dict[model_t](num_classes=n_cls, pretrained=model_path, input_tdim=input_tdim)
        print('==> done')
        return model
    if model_t == 'EfficientNet':
        image_size = (opt.dcase_target_frames, 128) if opt is not None else None
        model = model_dict[model_t](num_classes=n_cls, pretrained=model_path, image_size=image_size)
        print('==> done')
        return model
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    if opt.dataset in ['cifar100', 'tinyimagenet']:
        ...
        data = torch.randn(2, 1, opt.dcase_target_frames, 128)
    ...
    for epoch in range(cur_epoch, opt.epochs + 1):
        ...
        if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
            ...
            if test_acc > best_acc:
                ...
                save_file = os.path.join(opt.save_folder, '{}_best_{}.pth'.format(opt.model_s, opt.run_id))
